<div class="main-sidebar sidebar-style-2">
        <aside id="sidebar-wrapper">
          <div class="sidebar-brand">
            <a href="index.html">Vanss</a>
          </div>
          <div class="sidebar-brand sidebar-brand-sm">
            <a href="index.html">Vn</a>
          </div>
          <ul class="sidebar-menu">

            <li class="menu-header">Dashboard</li>
            <li class="dropdown">
              <a href="#" class="nav-link has-dropdown"><i class="fas fa-fire"></i><span>Dashboard</span></a>
            </li>

            <!--Master-->
            <li class="menu-header">Master</li>

            <li class="dropdown">
              <a href="/kategori" class="nav-link"><i class="fas fa-list"></i><span>Kategori</span></a>
            </li>
            <li class="dropdown">
              <a href="/penerbit" class="nav-link"><i class="fas fa-cloud-upload-alt"></i><span>Penerbit</span></a>
            </li>
            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-user"></i><span>Anggota</span></a>
            </li>
            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-book"></i><span>Buku</span></a>
            </li>

            <!--Transaksi-->
            <li class="menu-header">Transaksi</li>

            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-upload"></i><span>Pinjam</span></a>
            </li>
            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-download"></i><span>Kembali</span></a>
            </li>

            <!--User-->
            <li class="menu-header">User</li>

            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-users"></i><span>Anggota</span></a>
            </li>
            <li class="dropdown">
              <a href="#" class="nav-link"><i class="fas fa-user-secret"></i><span>Petugas</span></a>
            </li>

          </ul>
        </aside>
      </div>